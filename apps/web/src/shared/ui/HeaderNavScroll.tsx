...
function Logo({ withPill }: { withPill: boolean }) {
  ...
  return (
    <Link
      href="/"
      className={`group flex shrink-0 items-center gap-2 rounded-full ${pillClasses}`}
...
export function HeaderNavScroll({ categories }: HeaderNavScrollProps) {
  ...
  if (isCompact) {
    ...
    return (
      <div
        ...
        } transition-[padding,max-width] duration-300 ease-out px-3 md:px-4`}
      >
        ...
        <button
          ...
          className={`inline-flex items-center justify-center rounded-full bg-white/80 text-neutral-800 shadow-sm ring-1 ring-neutral-200/70 backdrop-blur-md active:bg-neutral-100 ${
            logoHasPill ? 'mt-4 h-11 w-11' : 'h-10 w-10'
          }`}
  ...
  // Десктопная версия: логотип + горизонтальное меню
  return (
    <div
      className="mx-auto flex h-full min-h-0 w-full max-w-6xl items-center justify-center gap-1 px-3 transition-[padding,max-width] duration-300 ease-out md:px-4"
    >
      <Logo withPill={!atTop} />
